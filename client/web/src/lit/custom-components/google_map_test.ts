...
describe('GoogleMap Component', () => {
  ...
  it('falls back to generic pin color when icon/iconColor is not provided',
     () => {
       ...
       expect(iconDiv.style.backgroundColor).toBe('rgb(120, 144, 156)');
     });

  it('uses lookup color when icon matches a known POI category', () => {
    const markerEl =
        new PlacePinMarker({label: 'Shop', icon: 'retail'}).getElement();
    ...
    expect(iconDiv).not.toBeNull();
    expect(iconDiv.style.backgroundColor).toBe('rgb(5, 151, 255)');
  });
     ...
     () => {
       const markerEl =
           new PlacePinMarker({
             label: 'Shop',
             icon:
                 '<svg xmlns="http://www.w3.org/2000/svg" width="28" height="32"><path d="M0 0h24v24H0z"/></svg>',
           }).getElement();
       const template =
           markerEl.querySelector('template') as HTMLTemplateElement;
       expect(template).not.toBeNull();
