...
export class GoogleMap extends A2uiLitElement<typeof GoogleMapApi> {
  ...
  private addMarkerAndLabel(threeDMarker: ThreeDMarker) {
  ...
  override render() {
    ...
    return html`
      <section aria-label="${UIStrings.MSG_MAP_VIEW}" style=${styleMap(style)}>
        <gmp-map-3d
          center="${lat},${lng},0"
          tilt="${tilt}"
          mode="${mode}"
