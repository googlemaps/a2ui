...
const MapPinSchema = z.object({
                        ...
                        iconColor: DynamicStringSchema.optional(),
                      }).strict();
  ...
  schema:
      z.object({
         ...
         mode: z.enum(['roadmap', 'satellite'])
                   ...
                   .optional(),
         anchorMarker:
             MapPinSchema.describe('The anchor marker location.').optional(),
...
export class GoogleMap extends A2uiLitElement<typeof GoogleMapApi> {
    ...
    if (Array.isArray(markers)) {
      return markers
          .map((marker: any) => ({
                 ...
                 icon: marker.icon as string | undefined,
                 iconColor: marker.iconColor as string | undefined,
  ...
  private createThreeDMarker(input: MarkerInput): ThreeDMarker {
    const markerId = `marker-${nextMarkerId++}`;
    return new ThreeDMarker({
      id: markerId,
      autofitsCamera: true,
      position: input.position,
      label: input.label,
      collisionBehavior: input.collisionBehavior,
      zIndex: input.zIndex,
    });
  }

  private addThreeDMarkerAndLabel(threeDMarker: ThreeDMarker) {
    const markerEl = threeDMarker.getElement();
    this.map3dElement.appendChild(markerEl);
    this.markers.push(markerEl);

    const labelEl = threeDMarker.getLabel();
    if (labelEl) {
      this.map3dElement.appendChild(labelEl);
      this.markers.push(labelEl);
    }
  }
  ...
  private async updateMarkers() {
    ...
    for (const {lat, lng, label, icon, iconColor} of markers) {
    ...
    if (anchorMarker && !routes.length) {
      const marker = this.createThreeDMarker({
        position:
      ...
      if (typeof google !== 'undefined' && google.maps && google.maps.marker &&
        ...
        marker.getElement().append(pin as any);
      }
      this.addThreeDMarkerAndLabel(marker);
    }
  }
