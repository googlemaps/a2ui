...
import {getPinColor, getPinIcon, PLACE_PIN_COLOR_LOOKUP, PLACE_PIN_ICON_LOOKUP, PLACE_PIN_MARKER_STYLES} from './place_pin_marker_constants';

export {getPinColor, getPinIcon, PLACE_PIN_COLOR_LOOKUP, PLACE_PIN_ICON_LOOKUP, PLACE_PIN_MARKER_STYLES,} from './place_pin_marker_constants';
...
export function createPlacePinMarkerTemplate(options: {
  ...
  const iconContent = getPinIcon(options.icon, options.iconColor);
  if (iconContent) {
    const customMarkerContentIcon = document.createElement('div');
    customMarkerContentIcon.classList.add('custom-marker-content-icon');

    const backgroundColor = getPinColor(options.icon, options.iconColor);
    customMarkerContentIcon.style.backgroundColor = backgroundColor;

    const parser = new DOMParser();
    const doc = (parser as any).parseFromString(iconContent, 'image/svg+xml');
    const svgElement = doc.documentElement;
    customMarkerContentIcon.appendChild(svgElement);
