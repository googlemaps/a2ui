...
class TestAgentOrchestration(unittest.IsolatedAsyncioTestCase):
  ...
  def test_template_registry_definitions(self):
    ...
    for bundle in agent.template_registry.bundles.values():
  ...
  def test_extractor_agent_mounts_tools_for_all_bundles(self):
    """Verifies that the extractor agent mounts a BaseTemplateTool for every bundle."""
    agent = MAUIAgentWithTemplates(base_url="http://test-url")
    extractor_agent = (
        agent._build_unified_extractor_agent()  # pylint: disable=protected-access
    )
    tools_by_name = {
        t.name: t for t in extractor_agent.tools if hasattr(t, "name")
    }
    for bundle in agent.template_registry.bundles.values():
      expected_name = bundle.render_tool_name
      self.assertIn(expected_name, tools_by_name)
      tool = tools_by_name[expected_name]
      self.assertIsInstance(tool, template_tool.BaseTemplateTool)
      self.assertEqual(tool.template_name, bundle.name)
      self.assertEqual(tool.schema_class, bundle.schema_cls)
      self.assertEqual(tool.description, bundle.effective_tool_description)
