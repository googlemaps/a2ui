...
class TestTemplateTools(unittest.IsolatedAsyncioTestCase):
  ...
  async def test_render_local_search_template_repairs_missing_heading(self):
    tool = RenderLocalSearchTemplateTool(schema_manager=self.schema_manager)

    # Missing heading, which the bundle schema synthesizes.
    args = {
  ...
  async def test_render_directions_template_repairs_missing_heading(self):
    tool = RenderDirectionsTemplateTool(schema_manager=self.schema_manager)
    ...
    self.assertEqual(heading_comp["text"], "### Route from A to B")

  async def test_render_directions_template_repairs_unroutable_travel_mode(
      self,
  ):
    tool = RenderDirectionsTemplateTool(schema_manager=self.schema_manager)
