...
class VertexGroundingExtractor:
  ...
  def _normalize_place_ids(
    ...
    if not isinstance(extracted_payload, dict):
      raise ValueError(
          "<data> block must be a JSON object, got"
          f" {type(extracted_payload).__name__}."
      )
    ...
    normalize_pin_place_id(extracted_payload.get("anchor_marker"))
...
"""
  ...
  async def generate_grounded_text(
      self,
      query: str,
      system_instruction: str = _GROUNDED_TEXT_BASE_INSTRUCTION,
  ) -> str:
    """Generates a plain-text grounded response using Vertex AI GwGM.

    Args:
        query: User query string.
        system_instruction: Optional instruction override.

    Returns:
        The generated text, or an empty string on error or safety block.
    """
    config_kwargs: dict[str, Any] = {
