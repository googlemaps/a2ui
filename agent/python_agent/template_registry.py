...
class TemplateBundle:
  ...
  @property
  def render_tool_name(self) -> str:
    """The function call name of the ADK tool generated to render this bundle."""
    return f"render_{self.name}_template"

  @property
  def effective_tool_description(self) -> str:
    """Description the model sees for this bundle's render tool.

    A bundle that does not write its own gets one derived from the archetype
    it already declares for the router, so the two cannot drift apart.
    """
    if self.tool_description:
      return self.tool_description
    return (
        f"Renders the {self.name} UI component. Use it when the request"
        f" matches: {self.archetype}"
    )
...
class TemplateRegistry:
  ...
  def _load_bundle(self, bundle_dir: pathlib.Path) -> None:
    ...
    bundle = TemplateBundle(
        ...
        empty_fallback_text=manifest.empty_fallback_text,
