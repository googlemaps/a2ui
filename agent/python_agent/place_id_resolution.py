...
#
# The rules are two-branch because whether the model can see a real Place ID
# depends on serving-side configuration. When it is off, no Place ID reaches the
# model, and the placeholder is the only option.
#
# Preferring the authoritative value is strictly better, but the model does not
# always get it, so we have to handle both cases.
PROMPT_RULES = """PLACE ID RULES:
Every `placeId` you emit MUST come from one of the two sources below. Always prefer source 1.
1. AUTHORITATIVE PLACE ID (preferred). If the Google Maps result for a place carries a place_id field -- written either as a `place_id: places/ChIJ...` line or as a `place_id='places/ChIJ...'` field inside a PerQueryResult -- copy that value verbatim into `placeId`. Do NOT reformat, abbreviate, or truncate it.
2. SYNTHETIC PLACEHOLDER (fallback). Use this if, and ONLY if, the Google Maps result for that place carries no place_id field:
- Format: "PLACE_ID_FOR_{Count}_{Exact Title}"
- Example: If the tool returns a place named "Chez Panisse" and gives no place_id, use "PLACE_ID_FOR_1_Chez Panisse". If it returns a second "Chez Panisse", use "PLACE_ID_FOR_2_Chez Panisse".
...
NEVER invent a `ChIJ...` value that the tool did not give you. If you have no place_id from the tool, the placeholder in rule 2 is the only acceptable output."""

_BRANDED_TITLE_SUFFIX = " - Google Maps"

# Slicing at len(prefix) - 3 keeps the "ChI" that every Place ID starts with.
_PLACE_ID_PREFIX = "places/ChI"
...
def normalize_place_id(raw_place_id: str) -> str:
  ...
  if raw_place_id.startswith(_PLACE_ID_PREFIX):
    return raw_place_id[len(_PLACE_ID_PREFIX) - 3 :]
  return raw_place_id
...
def resolve_place_ids(
  ...
  After substitution, any remaining Places API resource names are reduced to
  bare Place IDs, so neither branch of `PROMPT_RULES` can ship a resource name
  to the A2UI components.
  ...
  text = text.replace(_PLACE_ID_PREFIX, "ChI")
