...
class MAUIAgentWithTemplates(MAUIAgent):
  ...
  def _on_tool_error(
    ...
    if isinstance(tool, BaseTemplateTool) and isinstance(
...
2. After gathering information, you MUST call the appropriate template rendering tool to render the final response interface."""
  ...
  async def _run_extractor(
    ...
    template_tools = {
        ...
        if isinstance(t, BaseTemplateTool)
    }
    ...
    async for event in runner.run_async(
      ...
      if hasattr(event, "get_function_calls"):
        for fc in event.get_function_calls():
          target_tool = template_tools.get(fc.name)
  ...
  async def _run_extractor_and_merge(
    ...
    formatted_prompt = f"[TARGET_INTENT: {bundle.name}]\n{cleaned_query}"
    current_message = types.Content(
    ...
    rendered_parts, full_content_list, rendered_data = (
        await self._run_extractor(
            ...
            current_message,
            session_id,
            state_delta={"target_intent": bundle.name},
        )
    )
  ...
  async def stream(
    ...
    elif intent in self.template_registry.supported_intents:
