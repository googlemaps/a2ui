...
class MAUIAgentWithTemplates(MAUIAgent):
  ...
  def _build_unified_extractor_instruction(self) -> str:
    ...
    target_intent_rules = []
    for intent, tool_class in sorted(_INTENT_TOOL_CLASSES.items()):
      target_intent_rules.append(
          f"   - `[TARGET_INTENT: {intent}]`: Invoke"
          f" `{tool_class.name}`. Do NOT invoke any other template"
          " rendering tool."
      )
    ...
    workflow_rules = (
        ...
        " MUST strictly invoke the corresponding template rendering tool as"
        " your final step:\n"
        f"{target_intent_rules_text}\n"
        "3. Your final action MUST be this template rendering tool call. Do"
        " NOT emit raw JSON or `<a2ui-json>` blocks in your text response."
    )
  ...
  def _build_unified_extractor_agent(
    ...
    # Template tools handle template merging and use schema_manager only for
    # catalog validation. Do not call schema_manager.generate_system_prompt()
    # here, as it injects raw A2UI schema and <a2ui-json> output instructions.
