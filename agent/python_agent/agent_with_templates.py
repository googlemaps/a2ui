...
TEMPLATE_INFO: dict[IntentClass, TemplateInfo] = {
    IntentClass.LOCAL_SEARCH: TemplateInfo(
        ...
        skill_name="local-search-template-response",
    ...
    IntentClass.DIRECTIONS: TemplateInfo(
        ...
        skill_name="directions-template-response",
    ...
    for info in TEMPLATE_INFO.values():
      ...
      if skill_dir.exists():
        try:
          ...
          logger.warning("Failed to load skill '%s': %s", info.skill_name, e)
...
2. After gathering information, you MUST call the appropriate template rendering tool to render the final response interface."""
  ...
  async def _run_extractor_and_merge(
    ...
    target_hint = intent.name if intent else skill_name
