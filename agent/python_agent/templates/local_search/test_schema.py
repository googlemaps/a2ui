...
class TestLocalSearchSchema(unittest.TestCase):
  ...
  def test_local_search_extractor_schema_strips_markdown_heading(self):
    """Verifies that a markdown header prefix is stripped from the heading."""
    schema = LocalSearchExtractorSchema(
  ...
  def test_local_search_extractor_schema_synthesizes_heading_from_anchor(self):
    """Verifies that an omitted heading names the anchor marker."""
    schema = LocalSearchExtractorSchema(
        summary="Here are 5 transit stops.",
        center_lat=47.6205,
        center_lng=-122.3493,
        anchor_marker={"lat": 47.62, "lng": -122.35, "label": "Space Needle"},
        places=[{
            "placeId": "ChIJ111",
            "name": "Stop 1",
            "lat": 47.62,
    ...
    self.assertEqual(schema.heading, "Places near Space Needle")

  def test_local_search_extractor_schema_synthesizes_heading_without_anchor(
      self,
  ):
    """Verifies the heading falls back to a bare label with no anchor."""
    data = {
        "summary": "Here are 5 transit stops.",
        "center_lat": 47.6205,
        "center_lng": -122.3493,
    ...
    schema = LocalSearchExtractorSchema(**data)
    self.assertEqual(schema.heading, "Nearby Places")

  def test_local_search_extractor_schema_still_requires_places(self):
    """Verifies that heading leniency did not relax the other fields."""
