...
3.  **Compute Routes and Dispatch Response (Parallel Routing)**:
    ...
    *   Call `render_directions_template` with `DirectionsExtractorSchema`
        parameters (`summary`, `center_lat`, `center_lng`, `zoom`, `routes`,
        `travel_mode`).
...
`render_directions_template(summary="Driving from San Francisco to San Jose
...
`render_directions_template(summary="Walking from Central Park to Times Square
...
`render_directions_template(summary="Biking from Venice Beach to Santa Monica
...
`render_directions_template(summary="Take the 4 or 5 subway line south from
...
`render_directions_template(summary="Driving from Austin to San Antonio takes
