...
class ResolvePlaceIdsTest(unittest.TestCase):
  ...
  def test_resolves_global_list_numbered_placeholders_for_distinct_titles(self):
    text = (
        "PLACE_ID_FOR_1_Sushi Ginza Onodera, "
        "PLACE_ID_FOR_2_Shinjuku Sushi Hatsume, "
        "PLACE_ID_FOR_9_Sushi Dai"
    )
    chunks = [
        _chunk("Sushi Ginza Onodera - Google Maps", "places/ChIJ_onodera"),
        _chunk("Shinjuku Sushi Hatsume - Google Maps", "places/ChIJ_hatsume"),
        _chunk("Sushi Dai - Google Maps", "places/ChIJ_sushidai"),
    ]

    result, unresolved = place_id_resolution.resolve_place_ids(text, chunks)

    self.assertEqual(result, "ChIJ_onodera, ChIJ_hatsume, ChIJ_sushidai")
    self.assertEqual(unresolved, 0)

  def test_fallback_skips_duplicate_titles_when_both_misindexed(self):
    text = (
        "PLACE_ID_FOR_1_Tartine, "
        "PLACE_ID_FOR_2_Boudin, "
        "PLACE_ID_FOR_3_Starbucks, "
        "PLACE_ID_FOR_4_Starbucks"
    )
    chunks = [
        _chunk("Tartine - Google Maps", "places/ChIJ_tartine"),
        _chunk("Boudin - Google Maps", "places/ChIJ_boudin"),
        _chunk("Starbucks - Google Maps", "places/ChIJ_sbux_1"),
        _chunk("Starbucks - Google Maps", "places/ChIJ_sbux_2"),
    ]
    ...
    self.assertEqual(
        result,
        "ChIJ_tartine, ChIJ_boudin, "
        "PLACE_ID_FOR_3_Starbucks, PLACE_ID_FOR_4_Starbucks",
    )
...
class PromptContractTest(unittest.TestCase):
  ...
  def test_prompt_rules_describe_the_resource_name_the_parser_strips(self):
    # The authoritative branch tells the model to copy a `places/ChIJ...`
    # value verbatim, and resolve_place_ids strips exactly that prefix. If the
    # prompt were reworded to ask for a bare ID while the strip stayed, or vice
    # versa, resource names would ship to the client. Pin the shared form.
    self.assertIn(
        place_id_resolution._PLACE_ID_PREFIX,  # pylint: disable=protected-access
